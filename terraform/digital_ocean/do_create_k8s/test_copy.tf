...
resource "digitalocean_kubernetes_cluster" "k8s_clusters" {
  name   = var.cluster_name
  region = var.do_data_center
  ...
  node_pool {
    ...
    min_nodes  = 2
    max_nodes  = 3
    tags       = [var.cluster_name[count.index].index]
  }
}

resource "digitalocean_kubernetes_cluster" "test_again_k8s_clusters" {
  name   = var.cluster_name
  region = var.do_data_center
  count       = length((var.cluster_name))
  # HINT: If this breaks, you can use `var.do_k8s_slug_ver`, but uncomment it
