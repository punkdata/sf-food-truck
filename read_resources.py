                        ...
                        with open(file_path, 'r', encoding='utf-8') as f:
                            for line_num, line in enumerate(f, start=1):
                                ...
                                if inside_resource and match_resource_end.match(line):
                                    ...
                                    # After collecting the resource, check if it matches the condition
                                    if any((match_cnt.search(res_line) and not (match_bools0.search(res_line) or match_bools1.search(res_line))) or 
                                            (match_cnt_index.search(res_line) and not match_comments.search(res_line)) for res_line in resource_lines):

                                        # Ensure we don't print the same resource multiple times
                                        if current_file != last_file:
                                            fpath = f'--------------\nFile Path: {file_path}\n--------------\n'
                                            results_file.write(fpath)
                                            last_file = current_file

                                        # Write the entire resource block only once
                                        results_file.write('\n'.join(resource_lines) + "\n\n")
                                        total_count += 1

                                    # Reset resource_lines for the next resource block
                                    resource_lines = []
